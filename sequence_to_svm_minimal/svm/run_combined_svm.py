...
from features.stapep_columns import (
    STAPEP_COLS, QSAR_COLS, TEST_NAMES, SVM_PARAM_GRID as SVM_GRID,
    QSAR_TEST_NAME_MAP as QSAR_ID_MAP,
)
...
# ── Paths ──────────────────────────────────────────────────────────────────────
PATHS = {
    "amp_stapep"  : STAPEP_DIR / "stapled_amps_features.csv",
    "decoy_stapep": STAPEP_DIR / "stapled_decoys.csv",
    "amp_qsar"    : STAPEP_DIR / "qsar_stapled_amps.csv",
    "decoy_qsar"  : STAPEP_DIR / "qsar_stapled_decoys.csv",
    "test_stapep" : STAPEP_DIR / "test_stapled_features.csv",
    "test_qsar"   : STAPEP_DIR / "qsar_stapled_test.csv",
}
...
def pretrained_qsar_inference(test_qsar: pd.DataFrame) -> np.ndarray:
    """Run the 2016 PNAS pretrained SVM on test QSAR features.
    Returns P(AMP) array ordered by TEST_NAMES (NaN for missing rows).
    """
    ...
    for i, name in enumerate(TEST_NAMES):
        ...
        if row.empty:
            continue
        prob_arr[i] = float(model.predict_from_descriptors(row).prob_amp[0])
...
def main(cv_folds: int = 5):
    ...
    out_path = STAPEP_DIR / "test_combined_svm_predictions.csv"
