...
# Make utils/ and features/ importable regardless of cwd. This also fixes a
# pre-refactor bug where BASE = svm/ (not project root), so the data path
# resolved to svm/data/... which doesn't exist.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from utils.paths import STAPEP_DIR
from features.stapep_columns import STAPEP_COLS as ALL_SP_COLS, TEST_NAMES

# ── Feature sets ──────────────────────────────────────────────────────────────
NO_LOOP_COLS = [c for c in ALL_SP_COLS if c != "loop_percent"]

# ── Load training data ────────────────────────────────────────────────────────
amp = pd.read_csv(STAPEP_DIR / "stapled_amps_features.csv")
dec = pd.read_csv(STAPEP_DIR / "stapled_decoys.csv")
...
# ── Load test peptides ────────────────────────────────────────────────────────
test = pd.read_csv(STAPEP_DIR / "test_stapled_features.csv")
