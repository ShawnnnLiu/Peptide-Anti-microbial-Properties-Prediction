...
from utils.paths import PROJECT_ROOT as BASE, STAPEP_DIR as STAPEP
from utils.pretrained_svm import load_pretrained_svm
from features.stapep_columns import STAPEP_COLS, QSAR_COLS
...
def pretrained_qsar_scores():
    ...
    rows = [
        {d: QSAR_FEATURES[v].get(d, 0.0) for d in model.desc_names}
        for v in VARIANTS
    ]
    pred = model.predict_from_descriptors(pd.DataFrame(rows))
    return pred.prob_amp, pred.decision
