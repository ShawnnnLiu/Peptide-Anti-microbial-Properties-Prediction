...
# Make utils/ importable regardless of cwd. The propy3_vendor / propy3_src
# fallbacks below are kept for environments where propy is not system-installed.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))
from utils.paths import PROJECT_ROOT as SCRIPT_DIR, STAPEP_DIR, DESCRIPTORS_DIR as DESC_DIR, AAINDEX_DIR
...
# ---------------------------------------------------------------------------
# Bootstrap propy import (vendored fallbacks under descriptors/ kept for
# environments where propy isn't system-installed — non-existent paths are
# harmless in sys.path)
# ---------------------------------------------------------------------------
for p in [str(DESC_DIR / "propy3_vendor"), str(DESC_DIR / "propy3_src")]:
...
# Test-peptide name map. NOTE: keys are the QSAR-style names ("Buf(i+4)_12")
# distinct from the model-script TEST_NAMES list ("Buf12"). These two
# vocabularies are bridged by features.stapep_columns.QSAR_TEST_NAME_MAP.
TEST_NAMES = {
    1: "Buf(i+4)_12",
    2: "Buf(i+4)_13",
    3: "Buf(i+4)_13_Q9K",
