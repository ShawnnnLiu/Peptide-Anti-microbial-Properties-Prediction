...
from features.stapep_columns import (
    STAPEP_COLS as SP_COLS, QSAR_COLS,
    TEST_NAMES as TEST_ORDER, QSAR_TEST_NAME_MAP,
)
...
def load_feature_sets() -> dict[str, tuple[pd.DataFrame, pd.Series]]:
    ...
    """
    mic   = parse_ecoli_mic(STAPEP_DIR / "stapled_amps.csv")
    sp    = pd.read_csv(STAPEP_DIR / "stapled_amps_features.csv")
    qsar  = pd.read_csv(STAPEP_DIR / "qsar_stapled_amps.csv").rename(
...
def load_test_sets() -> dict[str, pd.DataFrame]:
    ...
    sp_test   = pd.read_csv(STAPEP_DIR / "test_stapled_features.csv")
    qsar_test = pd.read_csv(STAPEP_DIR / "qsar_stapled_test.csv")
